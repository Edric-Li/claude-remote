...
export class ChatGateway {
  ...
  @UseFilters(new WsExceptionFilter())
  async handleTaskAssign(@ConnectedSocket() client: Socket, @MessageBody() data: TaskAssignDto) {
    try {
...
export class AgentLoadBalancer {
  selectAgent(criteria: AgentSelectionCriteria): ConnectedAgent | null {
...
export class HealthController {
  ...
  async getAgentHealth(): Promise<AgentHealthSummary> {
    ...
    const healthChecks = await Promise.all(agents.map(agent => this.checkSingleAgent(agent)))
